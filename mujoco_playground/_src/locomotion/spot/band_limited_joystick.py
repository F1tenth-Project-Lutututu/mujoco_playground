"""Spot joystick task with an episode-long band-limited command."""
...
from mujoco_playground._src.locomotion.spot import joystick
...
class BandLimitedJoystick(joystick.Joystick):
  ...
  def __init__(self, *args, **kwargs):
    ...
    command = self._config.command_config
    self._band_lower = jp.asarray(
        [command.lin_vel_x[0], command.lin_vel_y[0], command.ang_vel_yaw[0]]
    )
    self._band_upper = jp.asarray(
        [command.lin_vel_x[1], command.lin_vel_y[1], command.ang_vel_yaw[1]]
    )
